...
<div class="mt-5 mx-3">
    <div class="float-end">
        <form action="{{route('parts.remove')}}" method="POST" data-parts="remove">
            @csrf
            @method('DELETE')
            <button type="submit" value="消去" {{empty($items)?' disabled':''}} data-dialog='{"title":"パーツを空にする","body":"パーツ内のメモを空にします。よろしいですか？"}' class="m-0 p-0 btn btn-link position-relative" data-bs-toggle="tooltip" data-bs-title="空にする">
