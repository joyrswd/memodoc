...
| 機能名       | リクエスト     | URI           | 機能概要         |
|------------|--------------|----------------|--------------|
|新規メモ入力|GET|/memo/create|メモ内容入力枠表示<br>  タグ登録枠表示メモ内容<br>  連続投稿チェック表示<br>  ＊Twiiter投稿チェック表示<br>  ＊投降後Twiiterフォームモーダル表示
...
|素材メモ登録|GET|/parts/{memo}/add|カート内限界数判定<br> 素材となるメモをセッションに保存|
